...
const sectionToRender = document.getElementById("main_recipes")
...
const createRecipeCard = (recipe, index) => {
    ...
    const { Name, url, Description, Author, imageURL } = recipe

    const limitDescription = limitCharacters(Description, 50)

    // Apply classes from Bootstrap 5
    ...
    cardLink.textContent = "Go to recipe"
    ...
    const containerInfoRecipe = document.createElement("div")
    containerInfoRecipe.style.display = "flex"
    containerInfoRecipe.style.justifyContent = "space-between"
    containerInfoRecipe.style.flexDirection = "column"

    containerInfoRecipe.appendChild(cardTitle)
    containerInfoRecipe.appendChild(cardAuthor)
    containerInfoRecipe.appendChild(cardText)

    cardBody.appendChild(containerInfoRecipe)
...
const renderRecipes = () => {
    recipes.forEach((recipe, index) => {
        ...
        sectionToRender.appendChild(recipeCard)
    })
}
